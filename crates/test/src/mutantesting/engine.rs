...
pub struct FuzzStrategyBuilder<Program, Args, Wit, Add = ()> {
    ...
    pub(crate) _strategy_storage: Option<BoxedStrategy<(Arguments, WitnessValues, Add)>>,
    pub(crate) _blueprint: Option<Box<dyn ContractFuzzStrategyBlueprint<Program, Args, Wit, AdditionalInput = Add>>>,
...
pub type FuzzTxBluepirintStep<AdditionalValue> =
    dyn Fn(&mut FinalTransaction, &FuzzContext, &Arguments, &WitnessValues, &AdditionalValue) + Send + Sync + 'static;

pub struct FuzzTxBlueprint<Program, Args, Wit, Add = ()> {
    pub(crate) steps: Vec<Arc<FuzzTxBluepirintStep<Add>>>,
    _phantom: PhantomData<(Program, Args, Wit)>,
}
    ...
    AdditionalInput: std::fmt::Debug + Clone + 'static,
{
    ...
    fn get_final_tx(
        ...
        (self.f)(context, args, wit, additional)
    }
}
...
impl<Args, Wit> Default for StrategyStorageBuilder<Args, Wit, (), Just<()>> {
    fn default() -> Self {
        Self::new()
    }
}
