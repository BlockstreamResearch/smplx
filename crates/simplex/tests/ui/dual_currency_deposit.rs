...
fn test_e2e_behaviour() -> Result<(), String> {
    ...
    let recovered_arguments =
        derived_dual_currency_deposit::DualCurrencyDepositArguments::from_arguments(
            &arguments_values,
        )?;
...
fn test_e2e_random_behaviour() -> Result<(), String> {
    for seed in 0..32 {
        ...
        let original_witness =
            derived_dual_currency_deposit::DualCurrencyDepositWitness::generate_witness_raw(
                &mut rng,
            );

        let witness_values = original_witness.build_witness();
        let recovered_witness =
            derived_dual_currency_deposit::DualCurrencyDepositWitness::from_witness(
                &witness_values,
            )?;
        assert_eq!(original_witness, recovered_witness);
        ...
        let original_arguments =
            derived_dual_currency_deposit::DualCurrencyDepositArguments::generate_arguments_raw(
                &mut rng,
            );
        ...
        rng = StdRng::seed_from_u64(seed);
        let rand_raw_witness_values =
            derived_dual_currency_deposit::DualCurrencyDepositArguments::generate_arguments(
                &mut rng,
            );
