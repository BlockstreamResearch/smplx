...
fn test_e2e_random_behaviour() -> Result<(), String> {
    for seed in 0..32 {
        ...
        let recovered_witness =
            derived_exotic_values::ExoticValuesWitness::from_witness(&witness_values)?;
        assert_eq!(original_witness, recovered_witness);

        rng = StdRng::seed_from_u64(seed);
        let rand_raw_witness_values =
            derived_exotic_values::ExoticValuesWitness::generate_witness(&mut rng);
        assert_eq!(witness_values, rand_raw_witness_values);

        rng = StdRng::seed_from_u64(seed);
        let original_arguments =
            derived_exotic_values::ExoticValuesArguments::generate_arguments_raw(&mut rng);
        ...
        rng = StdRng::seed_from_u64(seed);
        let rand_raw_witness_values =
            derived_exotic_values::ExoticValuesArguments::generate_arguments(&mut rng);
