...
fn test_e2e_random_value_generation_behaviour() -> Result<(), String> {
    for seed in 0..32 {
        ...
        {
            let witness_values = derived_exotic_values::ExoticValuesWitness::generate_witness(&mut rng);

            let witness_values = regenerate_witness_values(&witness_values, &mut rng);
            let _ = derived_exotic_values::ExoticValuesWitness::from_witness(&witness_values)?;
        }

        {
            let arguments_values = derived_exotic_values::ExoticValuesArguments::generate_arguments(&mut rng);

            let arguments_values = regenerate_arguments_values(&arguments_values, &mut rng);
            let _ = derived_exotic_values::ExoticValuesArguments::from_arguments(&arguments_values)?;
        }
    }
    Ok(())
}
