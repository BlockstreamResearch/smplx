...
use simplex::program::{RandomArguments, RandomWitness, Program};
...
fn test_e2e_random_behaviour() -> Result<(), String> {
    for seed in 0..32 {
        ...
        let original_witness = derived_array_tr_storage::ArrayTrStorageWitness::generate_witness_raw(&mut rng);
        ...
        let rand_raw_witness_values = derived_array_tr_storage::ArrayTrStorageWitness::generate_witness(&mut rng);
        assert_eq!(witness_values, rand_raw_witness_values);

        rng = StdRng::seed_from_u64(seed);
        let original_arguments = derived_array_tr_storage::ArrayTrStorageArguments::generate_arguments_raw(&mut rng);
        ...
        rng = StdRng::seed_from_u64(seed);
        let rand_raw_witness_values = derived_array_tr_storage::ArrayTrStorageArguments::generate_arguments(&mut rng);
...
fn test_e2e_random_value_generation_behaviour() -> Result<(), String> {
    for seed in 0..32 {
        ...
        {
            let witness_values = derived_array_tr_storage::ArrayTrStorageWitness::generate_witness(&mut rng);

            let witness_values = regenerate_witness_values(&witness_values, &mut rng);
            let _ = derived_array_tr_storage::ArrayTrStorageWitness::from_witness(&witness_values)?;
        }

        {
            let arguments_values = derived_array_tr_storage::ArrayTrStorageArguments::generate_arguments(&mut rng);

            let arguments_values = regenerate_arguments_values(&arguments_values, &mut rng);
            let _ = derived_array_tr_storage::ArrayTrStorageArguments::from_arguments(&arguments_values)?;
        }
    }
    Ok(())
}
