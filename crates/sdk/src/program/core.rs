...
/// A trait that defines and additional functionality required for handling a Simplicity-based
/// program for a `Program` value wrapper generated by `include_simf!` macro.
pub trait SimplexProgram {
    /// Generates the script public key (scriptPubKey) associated with the Simplicity program.
    fn get_script_pubkey(&self, network: &SimplicityNetwork) -> Script {
        self.get_program().get_script_pubkey(network)
    }

    /// Computes the script hash for the current program based on the provided network.
    fn get_script_hash(&self, network: &SimplicityNetwork) -> [u8; 32] {
        self.get_program().get_script_hash(network)
    }

    /// Retrieves a reference to the program associated with the current instance.
    fn get_program(&self) -> &Program;

    /// Compiles the provided program based on the given arguments and returns the compiled program.
    fn get_compiled_program(args: Arguments) -> CompiledProgram;

    /// Returns a mutable reference to the `Program` instance.
