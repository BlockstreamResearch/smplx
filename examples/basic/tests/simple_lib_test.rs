...
use simplex_example::artifacts::single_lib::SingleLibProgram;
use simplex_example::artifacts::single_lib::derived_single_lib::{SingleLibArguments, SingleLibWitness};

fn get_p2pk(context: &simplex::TestContext) -> (SingleLibProgram, Script) {
    let signer = context.get_default_signer();

    let arguments = SingleLibArguments {
        public_key: signer.get_schnorr_public_key().serialize(),
    };

    let p2pk = SingleLibProgram::new(arguments);
...
#[simplex::test]
fn basic_test_with_single_lib(context: simplex::TestContext) -> anyhow::Result<()> {
    let provider = context.get_default_provider();
