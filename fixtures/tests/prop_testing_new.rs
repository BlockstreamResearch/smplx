...
mod simple_storage_test_new {
    use super::*;
    use std::fmt::Debug;

    use anyhow::Result;

    use crate::simple_storage_test_new::init::SimpleStorageInit;
    use init::*;
    ...
    mod init {
        ...
        > ArgGenFuzzStrategy<Args, Wit> for SimpleStorageInit<Args, Wit>
        {
            fn get_strategy(
                &self,
                _test_context: std::sync::Arc<FuzzContext>,
        ...
        impl<Args, Wit> PstBuilder<Arguments, WitnessValues> for SimpleStorageInit<Args, Wit> {
            fn build_pst(
                ...
                args: &Arguments,
                wit: &WitnessValues,
            ) -> (PartiallySignedTransaction, Arguments, WitnessValues) {
                let base_gen = SimpleStorageInitBaseContext::default();
                let mut rng = simplex::rand::prelude::StdRng::seed_from_u64(0);
                let (_, pst, modified_args, modified_wit) =
                    base_gen.build_base_transaction_3(context, args.clone(), wit.clone(), &mut rng);
        ...
        > ArgGenFuzzStrategy2<Args, Wit> for SimpleStorageInit<Args, Wit>
        {
            fn get_strategy(
                &self,
                test_context: std::sync::Arc<FuzzContext>,
                ...
                (
                    ...
                    .prop_flat_map(move |((args, wit), old_value)| {
                        dbg!(&args, &wit, old_value, "mapping..");
                        ...
                        (
                            ...
                            old_value..max_amount,
                        )
                            .prop_map(move |(args, wit, old_value, new_value)| {
                                dbg!("Exexuting this propmaps ...");
                                dbg!(&args, &wit, old_value, new_value);
...

                                {
                                    ...
                                    args_typed.user =
                                        context.signer.as_ref().unwrap().get_schnorr_public_key().serialize();
                                }
    ...
    fn test_simple_storage_mint_path_with_pset() -> Result<()> {
        ...
        fuzz_engine.with_default_signer();
        fuzz_engine.run_with_check_4(SimpleStorageInitCheck);
        Ok(())
    }
}
